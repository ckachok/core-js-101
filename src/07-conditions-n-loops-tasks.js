...
function isCreditCardNumber(ccn) {
  const arrDigits = ccn.toString().split('').map(Number).reverse();
  let sum = arrDigits[0];

  for (let i = 1; i < arrDigits.length; i += 1) {
    let digit = arrDigits[i];
    if (i % 2 !== 0) {
      digit *= 2;
    }
    if (digit > 9) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < bracketSets.length; i += 1) {
    if (newStr.includes(bracketSets[i])) {
      newStr = newStr.replace(bracketSets[i], '');
      i = -1;
    }
  }
...
function getCommonDirectoryPath(paths) {
  const subDirs = paths[0].split('/');
  const subDirPaths = subDirs.reduce((acc, elem, index) => {
    const subDirPath = `${acc[acc.length - 1] || ''}${elem}${index !== subDirs.length - 1 ? '/' : ''}`;
    acc.push(subDirPath);
    return acc;
  }, []).reverse();
  return subDirPaths.find((path) => paths.every((subDirect) => subDirect.indexOf(path) === 0)) || '';
}
...
function getMatrixProduct(m1, m2) {
  const m3 = Array(m1.length).fill('').map(() => Array(m2[0].length).fill(''));
  for (let i = 0; i < m1.length; i += 1) {
    for (let j = 0; j < m2[0].length; j += 1) {
      m3[i][j] = m1[i].reduce((acc, cur, index) => acc + cur * m2[index][j], 0);
    }
  }
  return m3;
}
